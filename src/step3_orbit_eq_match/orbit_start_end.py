...
OUTPUT_DIR = Path(r"E:\tables\orbit_start_end")
#
#2-2.  Show the directory in the terminal.
...
#**************************************************************************************************
# [5.  Extracing orbit number(file name), starting and ending time.]
for f in tqdm(range(length_allfile_names), desc="Extract start/end time of ob", unit="ob"):
#   
# 5-1.  Importing orbit data from a csv file.
    df = pd.read_csv(allfile_names[f])
#
# 5-2.  Converting "datetime" column from object into datetime64(without timezone info)
    df["datetime"] = pd.to_datetime(df["datetime"], format="mixed", utc=True).dt.tz_localize(None)
#
# 5-3.  Converting format of datetime(format:datetime64 => object)
    df["datetime"] = df["datetime"].dt.strftime("%Y/%m/%d  %H:%M:%S")
#
# 5-4.  Converting new format datetime from object into datetime64
    df["datetime"] = pd.to_datetime(df["datetime"], format="%Y/%m/%d  %H:%M:%S")
#
# 5-5.  Extarcting start/end time of orbit file.    
    start_t = pd.to_datetime(df["datetime"].iloc[0])
    end_t = pd.to_datetime(df["datetime"].iloc[-1])
#
# 5-6.  Inserting orbit number(file name), starting and ending name into the list object whichi is
#      prepared in 4.
...
data.to_csv(OUTPUT_DIR / "orbit_start_end_ver2.csv", index=False)
#
#**************************************************************************************************
