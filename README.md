...
### 2. 地震軌道の抽出
#### 内容：
- デクラスタリング後の本震のみのカタログを読み取る。

- 各地震について、Step2出力（正規化後データ）から、**地震発生4時間以内**に**震央から距離330km以内**を通過する軌道を抽出し、衛星が震央からの最小距離を計算する。ただし、330km区間における衛星の地磁気緯度の絶対値が>65°を含む地震イベントは除外する。
    - 衛星位置(lat/lon)と震央(lat/lon)の距離を、WGS84楕円体を使用した**ヒュベニ公式**で計算する。
    - 地震発生4時間以内は、orbit_datetime が eq_time - 4h <= t <= eq_time を満たす区間

- 1地震イベントに対して、複数な軌道がある場合、最小距離が最も小さい軌道を1本採用。

- 地震_id、地震発生時刻、地震位置(緯度)、地震位置(経度),紐づけた軌道ファイル名、衛星が330km以内を通過開始時刻、通過終了時刻、軌道ファイルの最初時刻、軌道ファイルの最後時刻、および最小距離を新しいcsvファイル(地震-軌道紐づけ表)として出力する。
    - 距離≤330 kmとなる連続サンプル区間のうち、最小距離点（t0）を含む区間を採用し、その最初/最後のdatetimeを pass_time_start/end とする

    - 連続＝「行が連続して距離条件を満たす区間（欠損があれば分断）」
...
- 地震-軌道紐づけ表：
    ...
    - ヘッダ:
    ```["eq_id", "eq_time", "eq_lat", "eq_lon", "orbit_file", "pass_time_start", "pass_time_end", "orbit_datetime_start", "orbit_datetime_end", "closest_dis_km"]```
---
#### 確認ポイント：
- 紐づけなかった地震イベント数
