...
3. 各地震イベントについて、
    ...
    - 対応する地震イベントの震央からの距離が<=330kmの区間を探す。この区間があれば、そのまま候補リストに残し、この軌道の距離<=330 kmを満たす最初/最後のサンプル時刻、および衛星が距離<=330km区間内での震央からの最小距離を候補リストに追加する。
        - ただし、距離<=330 km区間（pass_time_start〜pass_time_end）内で abs(mlat)>65 が1点でもあれば、その地震イベントは以後すべての候補を見ずに除外する。
